...
public class VTreeParser {
    ...
    public void parse(VTree vTree){
        allVoxels = vTree;
        cTree = new CTree(vTree.getHeight()-3);
        parseToChunk(vTree, 0, 0, 0);
        formatContainers();
    }
    
    private void parseToChunk(VTree vTree, int x, int y, int z){
        int height = vTree.getHeight();
        if (height==2){
            parseAux(vTree, x, y, z);
            formatContainers();
            Mesh chunk = createChunk(currentSize);
            cTree.addChunk((x/64), (y/64), (z/64), chunk);
            resetContainers();
        } else {
